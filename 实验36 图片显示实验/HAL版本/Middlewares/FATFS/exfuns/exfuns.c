...
uint8_t exfuns_init(void)
{
    uint8_t i;
    uint8_t res = 0;
	printf("exfuns_init begin\r\n");
    for (i = 0; i < FF_VOLUMES; i++)
    {
        fs[i] = (FATFS *)mymalloc(AXI_SRAM123, sizeof(FATFS));   /* 为磁盘i工作区申请内存 */
		printf("fs[i] = (FATFS *)mymalloc(AXI_SRAM123, sizeof(FATFS)) end\r\n");
        if (!fs[i])break;
    }
    
#if USE_FATTESTER == 1  /* 如果使能了文件系统测试 */
	printf("mf_init() 初始化文件系统测试(申请内存) 开始 \r\n");
                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    fsrc = (FIL *)mymalloc(AXI_SRAM123, sizeof(FIL));    /* 申请内存 */
    fdst = (FIL *)mymalloc(AXI_SRAM123, sizeof(FIL));
    fbuf = (uint8_t *)mymalloc(AXI_SRAM123, 8192);
    ...
    myfree(AXI_SRAM123, fsrc); /* 释放内存 */
    myfree(AXI_SRAM123, fdst);
    myfree(AXI_SRAM123, fbuf);
    return res;
}
...
#define MAX_PATHNAME_DEPTH  512 + 1     /* 最大目标文件路径+文件名深度 */
    ...
    fddir = (DIR *)mymalloc(AXI_SRAM123, sizeof(DIR));   /* 申请内存 */
    finfo = (FILINFO *)mymalloc(AXI_SRAM123, sizeof(FILINFO));

    if (fddir == NULL || finfo == NULL)res = 100;

    if (res == 0)
    {
        pathname = mymalloc(AXI_SRAM123, MAX_PATHNAME_DEPTH);
        ...
        if (res == 0)
        {
            ...
            myfree(AXI_SRAM123, pathname);
        }
    }

    myfree(AXI_SRAM123, fddir);
    myfree(AXI_SRAM123, finfo);
...
#define MAX_PATHNAME_DEPTH 512 + 1  /* 最大目标文件路径+文件名深度 */
    ...
    srcdir = (DIR *)mymalloc(AXI_SRAM123, sizeof(DIR));  /* 申请内存 */
    dstdir = (DIR *)mymalloc(AXI_SRAM123, sizeof(DIR));
    finfo = (FILINFO *)mymalloc(AXI_SRAM123, sizeof(FILINFO));
    ...
    if (res == 0)
    {
        dstpathname = mymalloc(AXI_SRAM123, MAX_PATHNAME_DEPTH);
        srcpathname = mymalloc(AXI_SRAM123, MAX_PATHNAME_DEPTH);
        ...
        if (res == 0)
        {
            ...
            myfree(AXI_SRAM123, dstpathname);
            myfree(AXI_SRAM123, srcpathname);
        }
    }

    myfree(AXI_SRAM123, srcdir);
    myfree(AXI_SRAM123, dstdir);
    myfree(AXI_SRAM123, finfo);
    return res;
}
