...
void lcd_draw_hline(uint16_t x, uint16_t y, uint16_t len, uint16_t color)
{
	...
	LCD_SetAddress( x, y, x+len-1, y);	     // 设置坐标	

    LCD_WriteBuff(LCD_Buff,len);          // 写入显存
}
...
void lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint32_t color)
{
    ...
    height = ey - sy + 1;           /* 高度 */
    LCD_SetAddress( sx, sy, width, height);	// 设置坐标	
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   LCD_SPI_Transmit(&LCD_SPI, color, width*height) ;
...
//   LCD_SPI_Transmit(&LCD_SPI, LCD.Color, width*height) ;
	LCD_SPI_TransmitBuffer (&LCD_SPI,color, width*height);
