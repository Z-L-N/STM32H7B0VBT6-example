...
void norflash_init(void)
{
    uint8_t temp;
    ospi_init();                /* 初始化OSPI */
    norflash_ospi_disable();    /* 退出OPI模式(避免芯片之前进入这个模式,导致下载失败) */
    ...
    if (g_norflash_type == W25Q256)   /* SPI FLASH为W25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)     /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            ospi_send_cmd(FLASH_Enable4ByteAddr, 0, (0 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0); 
 ...
 * @retval      无
 */
static void norflash_ospi_disable(void)
{
    /* 写command指令,地址为0,无数据_8位地址_无地址_4线传输指令,无空周期,0个字节数据 */
    ospi_send_cmd(FLASH_ExitQPIMode, 0, (0 << 6) | (0 << 4) | (0 << 2) | (3 << 0), 0);
}
...
void norflash_write_enable(void)
{
    ...
    ospi_send_cmd(FLASH_WriteEnable, 0, (0 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0);
}
...
void norflash_write_disable(void)
{
    ...
    ospi_send_cmd(FLASH_WriteDisable, 0, (0 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0);
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    ospi_send_cmd(command, 0, (1 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0);
    ospi_receive(&byte, 1);
    return byte;
}
 ...
 */
uint32_t norflash_read_id(void)
{
    uint8_t temp[3];
    uint32_t deviceid;
    ospi_init();          /* 进行库函数调用前要先初始化 */
    /* SPI,读id,地址为0,单线传输数据_24位地址_单线传输地址_单线传输指令,无空周期,2个字节数据 */
    ospi_send_cmd(FLASH_JedecDeviceID, 0, (1 << 6) | (2 << 4) | (1 << 2) | (1 << 0), 0);
    ospi_receive(temp, 3);
...
void norflash_read(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    ospi_send_cmd(FLASH_FastReadQuad, addr, (3 << 6) | (g_norflash_addrw << 4) | (3 << 2) | (1 << 0), 6);
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    ospi_send_cmd(FLASH_PageProgramQuad, addr, (3 << 6) | (g_norflash_addrw << 4) | (1 << 2) | (1 << 0), 0);
...
void norflash_erase_chip(void)
{
    ...
    ospi_send_cmd(FLASH_ChipErase, 0, (0 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0);
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    ospi_send_cmd(FLASH_SectorErase, saddr, (0 << 6) | (g_norflash_addrw << 4) | (1 << 2) | (1 << 0), 0);
