...
{
	printf("mf_init() begin\r\n");
    fattester.file = (FIL *)mymalloc(AXI_SRAM123, sizeof(FIL));      /* 为file申请内存 */
    fattester.fatbuf = (uint8_t *)mymalloc(AXI_SRAM123, 512);        /* 为fattester.fatbuf申请内存 */
	
    if (fattester.file && fattester.fatbuf)
    {
		printf("mf_init() 内存申请成功\r\n");
        return 0;   /* 申请成功 */
    }
    else
    {
        mf_free();  /* 释放内存 */
		printf("mf_init() 内存申请失败\r\n");
        return 1;   /* 申请失败 */
    }
	printf("mf_init() end\r\n");
}
...
void mf_free(void)
{
    myfree(AXI_SRAM123, fattester.file);
    myfree(AXI_SRAM123, fattester.fatbuf);
}
